...
export class EditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    let id:string = this.route.snapshot.paramMap.get('ac');
    this.homologacionService.getServiceUrlFromNode().subscribe(url => {
      this.homologacionService.getCasco(id, url+'api/consulta/casco').subscribe((hom) => {
        ...
        this.data.image = hom.casImagen1;
        this.solRut = hom.solRut;
      })
    });
  }
  ...
  public guardarHomologacion() {
    ...
    else
    {
      ...
      if ( this.homologacion.homQrcode1 != undefined &&
        ...
        //console.log('homolog: ',this.homologacion);
        this.homologacionService.guardarHomologacion(this.homologacion).subscribe (
          homologacion => {
            swal({
              title: '<strong>Homologación actualizada Ok</strong>',
              imageAlt: 'QR',
              type: 'success',
